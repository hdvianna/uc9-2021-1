...
public class CatalogMapper implements Mapper<Catalog> {
    ...
    private Catalog map(ResultSet resultSet) throws SQLException {
        Brand brand = brandMapper.findById(resultSet.getInt("id_marca"));
        Size size = sizeMapper.findById(resultSet.getInt("id_tamanho"));
        Color color = colorMapper.findById(resultSet.getInt("id_cor"));
        Type type = typeMapper.findById(resultSet.getInt("id_tipo"));
